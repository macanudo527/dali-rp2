...
class PairConverterPlugin(AbstractPairConverterPlugin):
    def __init__(self, historical_price_type: str, api_key: Optional[str] = None , api_secret: Optional[str] = None) -> None:
        ...
        if api_key != None and api_secret != None:
            ...
            LOGGER.info("API key and API secret not provided for Coinbase Advanced Pair Converter Plugin. "
    ...
    def get_historic_bar_from_native_source(self, timestamp: datetime, from_asset: str, to_asset: str, exchange: str) -> Optional[HistoricalBar]:
        ...
        while retry_count < len(TIME_GRANULARITY):
            try:
                ...
                if self._authorized:
                    candle = self.client.get_candles(f'{from_asset}-{to_asset}', int(start.timestamp()), int(end.timestamp()), granularity).to_dict()['candles'][0]
                else:
                    candle = self.client.get_public_candles(f'{from_asset}-{to_asset}', int(start.timestamp()), int(end.timestamp()), granularity).to_dict()['candles'][0]
